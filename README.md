...
| `GROQ_API_KEY` | Transcript fetch, emails, summarizer, `founders.py`, `daily_summary.py`, `daily_summary_portco.py`, `main.py`, `get_facts`, `consolidator.py`, `create_notes.py` | Groq API key |
...
Reads one input file (a team transcript or a short instruction), extracts net-new **decisions**, **opinions**, and **actions**, and writes one timestamped note at the root of each uniquely matched deal folder.

```bash
python create_notes.py INPUT [--dry-run] [--refresh-identity]
```

**Example:**

```bash
python create_notes.py /tmp/team-sync.txt
python create_notes.py pass-on-clovie.txt --dry-run
```

| Flag | Description |
|------|-------------|
| `--dry-run` | Report notes without writing files |
| `--refresh-identity` | Force rebuild of each deal folder's `ai-generated/identity.json` |

Matching uses the `deals/` catalog only (not portcos). A fact is written only when `company_hint` uniquely matches a folder name or `identity.company_name`, or there is exactly one strong identity hit (company / alias / full founder name). If that fails, a unique high-similarity near-name (`Lightgood Energy` → `Lightgrid Energy`) is accepted; otherwise Groq may pick a single typo/variant from the catalog. No unique match or two+ candidates: that fact is skipped and reported. Multiple facts for the same deal become one file.

Net-new: if the deal folder already has top-level notes or `ai-generated/summary.md`, a second Groq call drops restatements of already-recorded facts. Contradictions and name-spelling variants are kept; input is treated as factual.

**Output:** `note_YYYYMMDD_HHMMSS.txt` at the deal folder root (local `now()` in the filename and the first line of the body). Example:
...
Stdout is human-readable (`WROTE` / `WOULD WRITE` / `SKIPPED` / `NO MATCH`). If nothing is written, prints `Did nothing. No notes generated.`
